/**
 * Comprehensive test to check that no rounds skip when players timeout
 * This test simulates timeouts in all rounds 1-10 and verifies proper progression
 */
...
// Test all rounds with timeouts
function testAllRoundsTimeout() {
  console.log("=== Testing All Rounds Timeout Behavior ===\n");
  
  console.log("Scenario:");
  console.log("- Rounds 1-10: All players timeout (no spells used in time)");
  console.log("");
  ...
  console.log(`Initial state: Round ${session.currentRound}`);
  
  // Simulate timeouts for all rounds 1-10
  for (let round = 1; round <= 10; round++) {
    const isOddRound = round % 2 === 1;
    console.log(`\n--- Round ${round} (Player ${isOddRound ? '1' : '2'} times out) ---`);
    ...
    // Verify that we're in the expected round
    const expectedRound = round + 1;
    if (session.currentRound === expectedRound) {
      console.log(`✓ Correctly transitioned to round ${expectedRound}`);
    } else {
      console.log(`✗ ERROR: Expected round ${expectedRound}, but got round ${session.currentRound}`);
    }
  }
  ...
  if (session.currentRound === 11) {
    console.log("✓ Test passed: All rounds progressed correctly without skipping");
  } else {
    console.log("✗ Test failed: Some rounds were skipped");
  }
