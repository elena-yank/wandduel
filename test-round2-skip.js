/**
 * Test to specifically check round 2 skipping issue
 * This test simulates the scenario where round 2 might be skipped
 */
...
// Test scenario that might cause round 2 to be skipped
function testRound2SkipScenario() {
  console.log("=== Testing Round 2 Skip Scenario ===\n");
  
  console.log("Scenario:");
  console.log("- Round 1: Player 1 gets a point (normal play)");
  console.log("- Round 2: Player 2 times out (should go to round 3)");
  console.log("");
  ...
  // Simulate round 1: Player 1 gets point (normal play)
  console.log("--- Round 1 (Player 1 gets point) ---");
  session = {
    ...session,
    ...simulateRoundCompletion(session, {
      attackSpellId: "attack-1", // Indicates normal play
      counterSuccess: false,
      player1Accuracy: 80,       // Player 1 has higher accuracy
      player2Accuracy: 60        // Player 2 has lower accuracy
    })
  };
  ...
  // Simulate round 2: Player 2 times out (this is where the bug might occur)
  console.log("\n--- Round 2 (Player 2 times out) ---");
  session = {
    ...session,
    ...simulateRoundCompletion(session, {
  ...
  // Check if we properly transitioned to round 3 (and not skipped it)
  if (session.currentRound === 3) {
    console.log("✓ Correctly transitioned to round 3");
  } else if (session.currentRound === 4) {
    console.log("✗ ERROR: Round 3 was skipped! Transitioned directly to round 4");
  } else {
    console.log(`? Transitioned to unexpected round ${session.currentRound}`);
  }
